...
export async function getPosts(maxNumber?: number): Promise<TPost[]> {
  let limitClause = "";
  ...
  return (maxNumber ? stmt.all(maxNumber) : stmt.all()) as TPost[];
}

export async function storePost(post: PostInput): Promise<RunResult> {
  const stmt = db.prepare(`
